...
def objective(trial,
    ...
    result_directory = os.path.join(rootDir, "Corn")
    ...
    if trial is None:
        ...
        if final == False:
            ...
            default_root_dir = os.path.join("../checkpoints", "Corn", "best")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "Corn", "best"), name="logs")
            dm.setup()
        else:
            callbacks = [model_checkpoint, early_stop_callback, progress_bar]
            default_root_dir = os.path.join("../checkpoints", "Corn", "final")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "Corn", "final"), name="logs")
            dm.setup("final")
    else:
        callbacks = [model_checkpoint, early_stop_callback, progress_bar]
        default_root_dir = os.path.join(rootDir, "Corn/")
        logger = TensorBoardLogger(save_dir=os.path.join(rootDir, "Corn/"), name="logs")
        dm.setup()

    lightning_model = TopoAge (model = modelname, loss = "Corn", headSize = headSize,
            freeze = freeze, flatten = False,
            learning_rate = lr, num_classes = NUM_CLASSES-1,
    ...
    if trial is None:
        if final == False:
            ...
            test_df.to_csv("../results/Corn.validation.csv")
            shutil.copyfile (model_checkpoint.best_model_path, "../results/Corn.best_train.model.ckpt")
            dump(early_stop_callback, "../results/Corn.earlystopping.dump")
        else:
            test_df["preds"] = preds
            mae = np.mean(np.abs(test_df["preds"]-test_df["Age"]))
            print ("final MAE on test:", mae)
            test_df = test_df[["preds", "Age"]]
            test_df.to_csv("../results/Corn.test.csv")
            shutil.copyfile (model_checkpoint.best_model_path, "../results/Corn.final.model.ckpt")
        return None
    else:
...
if __name__ == "__main__":
    print ("### Experiment using Corn Loss")
    ...
    print(df)
    df.to_csv(f"../results/optuna.results.Corn.csv")
