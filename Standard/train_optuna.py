...
def objective(trial,
    ...
    result_directory = os.path.join(rootDir, "Standard")
    ...
    if trial is None:
        ...
        if final == False:
            ...
            default_root_dir = os.path.join("../checkpoints", "Standard", "best")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "Standard", "best"), name="logs")
            dm.setup()
        else:
            callbacks = [model_checkpoint, early_stop_callback, progress_bar]
            default_root_dir = os.path.join("../checkpoints", "Standard", "final")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "Standard", "final"), name="logs")
            dm.setup("final")
    else:
        callbacks = [model_checkpoint, early_stop_callback, progress_bar]
        default_root_dir = os.path.join(rootDir, "Standard/")
        logger = TensorBoardLogger(save_dir=os.path.join(rootDir, "Standard/"), name="logs")
        dm.setup()
    ...
    if trial is None:
        if final == False:
            ...
            test_df.to_csv("../results/Standard.validation.csv")
            shutil.copyfile (model_checkpoint.best_model_path, "../results/Standard.best_train.model.ckpt")
            dump(early_stop_callback, "../results/Standard.earlystopping.dump")
        else:
            test_df["preds"] = preds
            mae = np.mean(np.abs(test_df["preds"]-test_df["Age"]))
            print ("final MAE on test:", mae)
            test_df = test_df[["preds", "Age"]]
            test_df.to_csv("../results/Standard.test.csv")
            shutil.copyfile (model_checkpoint.best_model_path, "../results/Standard.final.model.ckpt")
        return None
    else:
...
if __name__ == "__main__":
    ...
    print(df)
    df.to_csv(f"../results/optuna.results.Standard.csv")
