...
class TopoAge(pl.LightningModule):
    def __init__(self, learning_rate = 3e-4, imgSize = 224, num_classes = 1, model = None, headSize = [512, 256, 64],
        ...
        gclf = []
        gcSize = self.metaDataSize
        for j, z in enumerate(wideSize):
            gclf.append(nn.Linear(gcSize, z))
            gclf.append(nn.ReLU())
            gcSize = z
        self.gclassifier = nn.Sequential(*gclf)

        # create head
        clf = []
        clf.append(nn.Dropout(self.dropoutLevel))
        cSize = num_filters + gcSize
    ...
    def forward(self, x, g):
        ...
        og = self.gclassifier(g)
        fv = torch.cat( (og, fv), 1)
...
if __name__ == "__main__":
    # just browsing
    for model in ["effv2s", "densenet121", "resnet18", "resnet34", "resnet50"]:
        print ("\n\n###", model)
        for freeze in [-1, 0, 1, 2, 3, 4]:
            m = TopoAge(model = model, freeze = freeze, showStats = True, headSize = [1])
            z = summary(m, input_size=(1, 3, 224, 224), verbose = 0)
            print ("Freeze", freeze, z.trainable_params)
