...
def objective(trial,
    ...
    result_directory = os.path.join(rootDir, "StandardMeta")

    # retrain or tune?
    if trial is None:
        gamma = gamma/10
        headSize = [2**nd1, 2**nd2, 2**nd3]
        wideSize = [2**hd1, 2**hd2]
    else:
        n_epochs = 100
        ...
        hd1 = trial.suggest_int('hd1', 2, 6)
        hd2 = trial.suggest_int('hd2', 2, 6)
        wideSize = [2**hd1, 2**hd2]
    ...
    dm = ScoutViewDataModule(returnLabel = False, imgSize = imgSize, returnAsDict = False)
    if trial is None:
        # retrain same model
        if final == False:
            callbacks = [model_checkpoint, early_stop_callback, progress_bar]
            default_root_dir = os.path.join("../checkpoints", "StandardMeta", "best")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "StandardMeta", "best"), name="logs")
            dm.setup()
        else:
            callbacks = [model_checkpoint, early_stop_callback, progress_bar]
            default_root_dir = os.path.join("../checkpoints", "StandardMeta", "final")
            logger = TensorBoardLogger(save_dir=os.path.join("../checkpoints", "StandardMeta", "final"), name="logs")
            dm.setup("final")
    else:
        callbacks = [model_checkpoint, early_stop_callback, progress_bar]
        default_root_dir = os.path.join(rootDir, "StandardMeta/")
        logger = TensorBoardLogger(save_dir=os.path.join(rootDir, "StandardMeta/"), name="logs")
        dm.setup()

    lightning_model = TopoAge (learning_rate = lr, model = modelname, headSize = headSize,
            wideSize = wideSize,
            freeze = freeze, num_classes = 1, flatten = True, loss = "L1",
    ...
    if trial is None:
        if final == False:
            ...
            test_df.to_csv("../results/StandardMeta.validation.csv")
            shutil.copyfile (model_checkpoint.best_model_path, "../results/StandardMeta.best_train.model.ckpt")
            dump(early_stop_callback, "../results/StandardMeta.earlystopping.dump")
        else:
            test_df["preds"] = preds
            mae = np.mean(np.abs(test_df["preds"]-test_df["Age"]))
            print ("final MAE on test:", mae)
            test_df = test_df[["preds", "Age"]]
            test_df.to_csv("../results/StandardMeta.test.csv")
            shutil.copyfile (model_checkpoint.best_model_path, "../results/StandardMeta.final.model.ckpt")
        return None
    else:
...
if __name__ == "__main__":
    ...
    print(df)
    df.to_csv(f"../results/optuna.results.StandardMeta.csv")
